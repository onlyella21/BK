  ...
  function Header(){
    ...
    $this->Image('logo.jpeg',90,14,30);
     ...
     //line break 5mm
      $this->Ln(5);
      $this->SetFont('Arial','B','16');
      $this->Cell(270,7,'Laporan Point Semua Siswa',0,1,'C');
...
include'koneksi.php';
$kasus=mysqli_query($koneksi,"SELECT * FROM tab_point_siswa as p JOIN tab_siswa as s ON p.nis=s.nis JOIN tab_kelas as k ON s.id_kelas=k.id_kelas  JOIN tab_guru as g ON k.id_guru=g.id_guru ORDER BY point_akhir ASC") or die (mysqli_error($koneksi));
...
$min=mysqli_query($koneksi,"SELECT * FROM tab_point_siswa as p JOIN tab_siswa as s ON p.nis=s.nis JOIN tab_kelas as k ON s.id_kelas=k.id_kelas  JOIN tab_guru as g ON k.id_guru=g.id_guru ORDER BY point_akhir ASC LIMIT 1") or die (mysqli_error($koneksi));
