...
if(@$_POST['cetak']){
  $awal=mysqli_real_escape_string($koneksi,$_POST['txt_tanggalawal']);
  $akhir=mysqli_real_escape_string($koneksi,$_POST['txt_tanggalakhir']);
  $a=date('d F Y',strtotime($awal));
  $h=date('d F Y',strtotime($akhir));
$no=1;


include'koneksi.php';
$kasus=mysqli_query($koneksi,"SELECT * FROM tab_konsultasi as u JOIN tab_siswa as s ON u.nis=s.nis JOIN tab_kelas as k ON s.id_kelas=k.id_kelas WHERE tgl_konsultasi between '$awal' and '$akhir' ORDER BY tgl_konsultasi ASC") or die (mysqli_error($koneksi)) ;

$pdf->SetFont('Arial','B','10');
$pdf->Cell(270,7,'Periode '.$a.' sampai '.$h,0,1,'C');
