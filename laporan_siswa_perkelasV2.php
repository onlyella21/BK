...
include'koneksi.php';
if(@$_POST['cetak']){
        $id=mysqli_real_escape_string($koneksi,$_POST['kode_kelas']);
        $maha=mysqli_query($koneksi,"SELECT * FROM tab_siswa as s JOIN tab_kelas as k ON s.id_kelas=k.id_kelas where s.id_kelas='$id'");
...
$pdf->Cell(190,7,'DAFTAR SISWA SMK ITIKURIH HIBARNA ',0,1,'C');
$pdf->Cell(190,7,'KELAS: '.$ko,0,1,'L');
...
$pdf->Cell(35,8,'Nama Siswa',1,0,'C');
$pdf->Cell(27,8,'Jenis Kelamin',1,0,'C');
$pdf->Cell(80,8,'Alamat',1,0,'C');
$pdf->Cell(30,8,'No Telpon',1,1,'C');
...
$pdf->SetFont('Arial','','10');
 ...
 if(@$_POST['cetak']){
        $id=mysqli_real_escape_string($koneksi,$_POST['txtpilih']);
        $maha=mysqli_query($koneksi,"SELECT * FROM tab_siswa as s JOIN tab_kelas as k ON s.id_kelas=k.id_kelas where s.id_kelas='$id' OR s.angkatan='$id'");
...
$pdf->Cell(80,8,$row['alamat'],1,0);
$pdf->Cell(30,8,$row['no_telp_siswa'],1,1);
