
<html lang="en">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    
    <title>Aplikasi BK</title>

    <!-- Bootstrap core CSS -->
    <link href="assets/css/bootstrap.css" rel="stylesheet">

    <!-- Add custom CSS here -->
    <link href="assets/css/sb-admin.css" rel="stylesheet">
    <link rel="stylesheet" href="assets/font-awesome/css/font-awesome.min.css">
     <!-- JavaScript -->
    <script src="assets/js/jquery.js"></script>
    <script src="assets/js/bootstrap.js"></script>
  </head>

<body>
<?php
require_once('koneksi.php');

?>
 <div id="wrapper">
<?php 
include"header.php";
?>

   </div><!-- /.navbar-collapse -->
      </nav>
 <div id="page-wrapper">
<div class="row" id="contentInput" >
          <div class="col-lg-12">
            <h1>Laporan Kasus Siswa</h1>
            <ol class="breadcrumb">
              <li class="active"> Laporan</li>
              <li class="active">Laporan Kasus Siswa</li>
            </ol>
          </div>
      </div>
                  <div class="panel panel-info">
          <div class="panel-heading-none">
          </div>
          <div class="panel-body">
                    <a   href="laporan_semua_kasus.php" type="button" class="btn btn-success"><i class="fa fa-edit"></i>Cetak Semua Data Kasus Siswa</a>
               <hr>     
           

          <!-------------form berdasarkan kelas-->
         <h4>Per Kelas dan Bulan</h4>
          <form method="POST" action="laporan_kasus_perkelas.php" id="perkelas">
          <div class="form-group form-group-sm">
                 
                 <div class="form-group form-group-sm">
                  <label class=" control-label col-sm-2" > Pilih Kelas</label>
                   <div class="col-sm-2">
                    <select name="id_kelas" class="form-control" id="id_kelas"required>
                    <option value="">--Pilih--</option>
                    <?php
                $query="SELECT * FROM tab_kelas";  
                 $data=mysqli_query($koneksi,$query);

                  if(mysqli_num_rows($data) > 0){

                    while($row=mysqli_fetch_assoc($data)){
                ?>
              <option value="<?php echo $row['id_kelas']?>"><?php echo $row['kode_kelas']?></option>

                <?php
                 }  
                 } 
                  ?>
                    </select>
                   
                  </div>
                 </div>
                  
              
              </div>
               <label class=" control-label col-sm-2" >Pilih Tanggal</label>
                   <div class="col-sm-2">
                    <input type="date" class="form-control" name="txt_tanggalawal" id="txt_tanggalawal" placeholder="Dari Tanggal">
                  </div>
                <label class=" control-label col-sm-2" >Sampai Tanggal</label>
                <div class="col-sm-2">
                    <input type="date" class="form-control" name="txt_tanggalakhir" id="txt_tanggalakhir">
                </div>

                 <div class="col-sm-">
                   <input type="submit" align="right" class="btn btn-danger"  value="Cetak" name="cetak" id="cetak">
                </div>
 </form>
 <!---------------------Per Siswa dan periode----------------------->
 <hr> 
 <hr>  
  <h4>Per Siswa dan Tanggal</h4>

       <form method="POST" action="" id="carinis">
          <div class="form-group form-group-sm">
                 
                <div class="form-group form-group-sm">
                    <label class=" control-label col-sm-2" > Pilih Siswa</label>
                   <div class="col-sm-3">
                    <input type="text" class="form-control" name="txtsiswa" id="txtsiswa" placeholder="NIS atau Nama Siswa">
                </div>
         
         
                    <input type="submit" align="right" class="btn btn-success"  value="Cari" name="cari" id="cari">
                    
                </div>
              </div>
              </form>
              <?php
                if(ISSET($_POST['cari'])){
                   $cari=mysqli_real_escape_string($koneksi,$_POST['txtsiswa']);
                   $perintah="SELECT * FROM tab_siswa  WHERE  nis  LIKE '%$cari%' OR nama_siswa LIKE '%$cari%'";
               $da=mysqli_query($koneksi,$perintah)or die (mysqli_error($koneksi));
               if(mysqli_num_rows($da) > 0){
               $r=mysqli_fetch_assoc($da);
               
              ?>
               <form method="POST" action="laporan_kasus_persiswa.php" id="pertgl">
                 <div class="form-group form-group-sm">
                    <label class=" control-label col-sm-2" > NIS</label>
                   <div class="col-sm-3">
                    <input type="text" class="form-control" name="nis" id="nis" readonly value="<?php echo $r['nis'];  ?>">
                </div>
              </div>
               <label class=" control-label col-sm-2" > Nama Siswa</label>
                   <div class="col-sm-3">
                    <input type="text" class="form-control" name="nama" id="nama" readonly value="<?php echo $r['nama_siswa']; ?>">
                </div>
              </div>
               <label class=" control-label col-sm-2" >Pilih Tanggal</label>
                   <div class="col-sm-3">
                    <input type="date" class="form-control" name="txt_tanggalawal" id="txt_tanggalawal" placeholder="Dari Tanggal">
                  </div>
                <label class=" control-label col-sm-2" >Sampai Tanggal</label>
                <div class="col-sm-3">
                    <input type="date" class="form-control" name="txt_tanggalakhir" id="txt_tanggalakhir">
                </div>

                 <div class="col-sm-">
                   <input type="submit" align="right" class="btn btn-danger"  value="Cetak" name="cetak" id="cetak">
<hr>
 <?php

}
else{
  echo"<script>alert('Maaf data tidak ditemukan')</script>";
      echo"<script>location='laporan_kasus.php';</script>";
}
}
?>
 
 </form>
 <hr>
 <hr>
<h4>Per Pelanggaran dan Tanggal</h4>
          <form method="POST" action="laporan_kasus_perpelanggaran.php" id="perkelas">
          <div class="form-group form-group-sm">
                 
                  <label class=" control-label col-sm-2" > Pilih pelanggaran</label>
                           
                    <div class="col-sm-6">
                       
                  
              
                 <?php
                $query="SELECT * FROM tab_pelanggaran";  
                 $data=mysqli_query($koneksi,$query);
                 $jsArray="var pel=new Array();\n";
                
                    echo'<select name="cbpel" onchange="changeValue(this.value)" class="form-control">'; 
                      
                    echo'<option value="">---Pilih---</option>';
                    while($row=mysqli_fetch_array($data)){
                    echo'<option value="'.$row['id_pelanggaran'].'">'.$row['nama_pelanggaran'].'</option>';
                    $jsArray.="pel['".$row['id_pelanggaran']."']={nama:'".addslashes($row['nama_pelanggaran'])."',po:'".addslashes($row['pengurangan_point'])."'};\n";
                    }
                echo'</select>';
                    ?>
                    
                </div>
                <!--untuk nama_pelanggaran dan point pelanggaran-->
                <input type="hidden" class="form-control" name="nama_pel" id="nama_pel">
                <input type="hidden" class="form-control" name="point_pel" id="point_pel">
                <script type="text/javascript">
                  <?php echo $jsArray;?>
                  function changeValue(id){
                    document.getElementById('nama_pel').value=pel[id].nama;
                    document.getElementById('point_pel').value=pel[id].po;
                  };
                </script>
                </div>
                <br>
                <br>
                <label class=" control-label col-sm-2" >Pilih Tanggal</label>
                   <div class="col-sm-3">
                    <input type="date" class="form-control" name="txt_tanggalawal" id="txt_tanggalawal" placeholder="Dari Tanggal">
                  </div>
                <label class=" control-label col-sm-2" >Sampai Tanggal</label>
                <div class="col-sm-3">
                    <input type="date" class="form-control" name="txt_tanggalakhir" id="txt_tanggalakhir">
                </div>
                  
                  <div class="col-sm-2">
                   <input type="submit" align="right" class="btn btn-danger"  value="Cetak" name="cetak" id="cetak">
                  

             